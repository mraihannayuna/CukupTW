...
import com.example.cukuptw.databinding.FragmentInternationalBinding
...
class InternationalFragment : Fragment() {
    ...
    lateinit var binding: FragmentInternationalBinding
    private val InternationalViewModel: NewsViewModel by viewModels {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        InternationalViewModel.getSindonewsInternationalNews()

        InternationalViewModel.sindonewsInternationalNews.observe(viewLifecycleOwner) { dataNews ->
            dataNews?.data?.let { mAdapter.setData(it.posts) }
        }

        binding.rvRepublikaInternational.apply {
            adapter = mAdapter
            layoutManager = LinearLayoutManager(requireContext())
        }

        InternationalViewModel.isLoading.observe(viewLifecycleOwner){
            isLoading(it)
        }
